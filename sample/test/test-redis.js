...
const logger = getLogger('sample-redis');
logger.info('Starting Redis Cluster connection sample');

// Function to sample Redis connectivity
  ...
  try {
    ...
    redis.on('node error', (err, node) => {
      if (node && node.options) {
        logger.error(
          `Redis Cluster Node ${node.options.host}:${node.options.port} Error: ${err.message}`
        );
      } else {
    ...
    // Set a sample key
    logger.info('Setting sample key in cluster');
    await redis.set('sample-key', 'Hello Redis Cluster!');
    ...
    // Clean up
    await redis.quit();
    logger.info('Redis Cluster sample completed successfully');
  } catch (error) {
