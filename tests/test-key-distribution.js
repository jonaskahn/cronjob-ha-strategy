...
const logger = getLoggerInstance('test-key-distribution');
logger.info('Starting Redis Cluster key distribution test');

// Function to test if keys are properly accessible across the cluster
async function testKeyDistribution() {
  const ports = [6379, 6380, 6381];
  const connections = [];
  const testKeys = [
    'test-key-0',    // Will likely be stored on one node
    'test-key-1000', // Will likely be stored on a different node
    'test-key-5000', // Will likely be stored on a different node
    'test-key-10000' // Will likely be stored on a different node
  ];
  ...
  try {
    ...
    // First set all test keys from node 1 (primary node)
    logger.info('--- Setting test keys from primary node ---');
    ...
    for (const conn of connections) {
      ...
      for (const key of testKeys) {
        try {
          ...
          if (value) {
            logger.info(`Node ${conn.port}: Successfully retrieved key "${key}" with value: "${value}"`);
          } else {
          ...
          if (err.message.includes('MOVED')) {
            ...
            if (redirectMatch) {
              logger.info(`Node ${conn.port}: Key "${key}" is located on node ${redirectMatch[1]}:${redirectMatch[2]}`);
            } else {
    ...
    // Now test with the Cluster client to ensure it can access all keys
    ...
    logger.info('Redis Cluster key distribution test completed');
  } catch (error) {
