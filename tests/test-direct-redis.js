...
const logger = getLoggerInstance('test-direct-redis');
logger.info('Starting direct Redis connection test');

// Function to test Redis connectivity directly to each node
async function testDirectRedisConnection() {
  ...
  try {
    ...
    logger.info('Direct Redis connection test completed');
  } catch (error) {
